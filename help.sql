...
/**
 *  ОПАСНЫЕ СЦЕНАРИИ: VACUUM FULL
 *
 *  Когда выгорешь, или если твою работу не будут ценить, запусти под postgres (только postgres)  --VACUUM FULL;
 *  на базе > 500Gb это ядерная бомба 500 мегатон! 
 *  она будет работь целый день может и дольше, блокировка таблиц особено с (документами, полями документов, визитами, рецептами, адресами) - ну ты понял 
 *  будут ложить базу (из-за обильного sql-ddos пользователей)... но обиднее всего что после вылета postgres... а я гарантирую он вылетит!
 *  сломается структура той таблицы на которой он вылетит и (вероятно потом тебе придется ее востанавливать из быкапа)
 *  после постгрес работать уже НЕ БУДЕТ
 * 
 *
 *  ЧТО ДЕЛАТЬ ЕСЛИ ПЕРЕД ТОБОЙ УСПЕЛИ ИЗВИНИТЬСЯ!
 *  (срочно стопай всех пользователей кроме postgres команда у тебя есть выше, но лучше поставить запрет на любые коннекты кроме postgres)
 *  сообщи. что в данный момент запущены средства диагностики и их нельзя прерывать, ОЖИДАЙТЕ!
 *  предложи выбор гарантированная потеря данных за день, но чуть меньше ожидание(и востанови с бэкапа) / или стиснуть зубы и терпеть пока база сама починится;)
 * 
 * 
 *  PS если ты юмора не понял. то вот Вам обьяснение: прочитайте официальную документацию PostgreSQL
 * 	"и там настоятельно не рекомендуют использовать эти команды"
 *  Здесь я лишь хотел показать яркий пример, что опасно доверять пользователю postgres, и что нужно проверять бэкапы
 **/
...
/* ОПАСНЫЕ КОМАНДЫ, НО НЕ НАСТОЛЬКО КАК VACUUM ;)   */
...
-- Полностью закрыть базу your_database для всех, кроме суперпользователей (! ну ты это, не выполняй такое, без острой необходимости)
-- иногда может стать полезно
